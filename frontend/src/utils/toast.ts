...
function getToastContainer() {
  if (!toastContainer) {
    ...
    toastContainer.style.cssText = `
      ...
      gap: 0;
      pointer-events: none;
      max-width: ${isMobile ? 'calc(100vw - 24px)' : '420px'};
    `
...
export function showToast(message: string, type: ToastType = 'info') {
  ...
  toast.style.cssText = `
    ...
    border-left: ${isMobile ? '4px' : '5px'} solid ${colors.border};
  ...
  const closeBtnSize = isMobile ? '20px' : '24px';
  
  toast.innerHTML = `
    <div style="color: ${colors.icon}; flex-shrink: 0;">
      ${toastIcons[type].replace('width: 24px; height: 24px;', `width: ${iconSize}; height: ${iconSize};`)}
    </div>
    <div style="flex: 1; font-size: ${fontSize}; color: #1f2937; line-height: 1.5; font-weight: 500; overflow: hidden; text-overflow: ellipsis;">
      ${message}
    </div>
...
function updateToastPositions(container: HTMLElement) {
  ...
  const toasts = Array.from(container.children).filter(
    (t) => !closingToasts.has(t as HTMLElement),
  ) as HTMLElement[]
  const maxVisible = 3
...
function expandToasts(container: HTMLElement) {
  ...
  const isMobile = window.innerWidth <= 640
  const toastHeight = isMobile ? 58 : 68 // Smaller height on mobile
...
function closeToast(toast: HTMLElement, container: HTMLElement) {
  ...
  toast.style.transform = `translateX(${isMobile ? '120vw' : '450px'}) scale(0.9)`
  toast.style.opacity = '0'
