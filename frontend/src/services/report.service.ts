...
export const reportService = {
    ...
    async revenueTimeseries(params: DateRange): Promise<RevenuePoint[]> {
        if (!USE_MOCK) {
            const { data } = await api.get('/admin/reports/revenue/', { params: { ...params, type: 'timeseries' } })
            return data
        }
        const now = new Date()
    ...
    async revenueByGateway(params: DateRange): Promise<RevenueByGateway[]> {
        if (!USE_MOCK) {
            const { data } = await api.get('/admin/reports/revenue/', { params: { ...params, type: 'by-gateway' } })
            return data
        }
    ...
    async revenueTopCourses(params: DateRange): Promise<RevenueTopCourse[]> {
        if (!USE_MOCK) {
            const { data } = await api.get('/admin/reports/revenue/', { params: { ...params, type: 'top-courses' } })
            return data
        }
    ...
    async userKpis(params: DateRange): Promise<UserKPIs> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/users/', { params: { ...params, type: 'kpis' } }); return data }
        return { dau: 812, mau: 4321, newUsers: 213, activeUsers: 2760 }
    },
    async userSeries(params: DateRange): Promise<UserSeriesPoint[]> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/users/', { params: { ...params, type: 'timeseries' } }); return data }
        const now = new Date()
        const from = params.from ? new Date(params.from) : new Date(now.getTime() - 29 * 864e5)
        const to = params.to ? new Date(params.to) : now
    ...
    async userByRole(params: DateRange): Promise<UserByRole[]> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/users/', { params: { ...params, type: 'by-role' } }); return data }
        return [
            { role: 'admin', count: 8 },
    ...
    async learningKpis(params: DateRange): Promise<LearningKPIs> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/learning/', { params: { ...params, type: 'kpis' } }); return data }
        return { avgCompletion: 62, avgScore: 74, avgTimeSpentMin: 38 }
    },
    async completionSeries(params: DateRange): Promise<CompletionPoint[]> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/learning/', { params: { ...params, type: 'completion' } }); return data }
        const now = new Date()
        const from = params.from ? new Date(params.from) : new Date(now.getTime() - 29 * 864e5)
        const to = params.to ? new Date(params.to) : now
    ...
    async scoreBySubject(params: DateRange): Promise<ScoreBySubject[]> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/learning/', { params: { ...params, type: 'score-by-subject' } }); return data }
        return [
    ...
    async atRiskStudents(params: DateRange): Promise<AtRiskRow[]> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/learning/', { params: { ...params, type: 'at-risk' } }); return data }
    ...
    async contentKpis(params: DateRange): Promise<ContentKPIs> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/content/', { params: { ...params, type: 'kpis' } }); return data }
    ...
    async viewsBySubject(params: DateRange): Promise<ViewsBySubject[]> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/content/', { params: { ...params, type: 'views-by-subject' } }); return data }
        return [
    ...
    async topContents(params: DateRange): Promise<TopContentRow[]> {
        if (!USE_MOCK) { const { data } = await api.get('/admin/reports/content/', { params: { ...params, type: 'top' } }); return data }
        return Array.from({ length: 10 }).map((_, i) => ({
