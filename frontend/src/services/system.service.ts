...
export const systemService = {
    ...
    async updateConfig(payload: Partial<SystemConfig>) {
        if (!USE_MOCK) return api.post('/admin/system/config/', payload)
        return Promise.resolve({ ok: true, version: (payload as any)?.version ?? 13 })
    },

    async sendTestEmail(to: string) {
        if (!USE_MOCK) return api.post('/admin/system/test-email/', { email: to })
    ...
    async createBackup(notes?: string) {
        if (!USE_MOCK) return api.post('/admin/system/backups/', { type: 'manual', notes })
        return Promise.resolve({ ok: true, id: `bk-${Math.floor(Math.random() * 99999)}` })
    },

    async restoreBackup(id: string) {
        if (!USE_MOCK) return api.post(`/admin/system/restore/`, { backupId: id })
        return Promise.resolve({ ok: true })
    },

    async listConfigAudit(): Promise<ConfigAuditItem[]> {
        if (!USE_MOCK) {
            const { data } = await api.get('/admin/system/audit/')
            return data
        }
